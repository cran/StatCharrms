...
if (TestDirection=='Increasing' | TestDirection=='Both'){  #Increasing Test
	...
	Test = TRUE
	while (Test == TRUE){
		JTInc<-jonckheere.test(DataU[ ,Response],as.ordered(DataU[ ,Treatment]),nperm=50000,alternative='increasing') #50000
		#JTInc<-JT(DataU[ ,Response],as.ordered(DataU[ ,Treatment]),alternative='increasing') #50000
		...
		if (length(unique(DataU[ ,Treatment]))==1){
			Test = FALSE
		}
...
if (TestDirection=='Decreasing' | TestDirection=='Both'){ #Decreasing Test
	...
	Test = TRUE
	while (Test == TRUE){
		JTDec<-jonckheere.test(DataU[ ,Response],as.ordered(DataU[ ,Treatment]),nperm=50000,alternative='decreasing') #50000
		#JTDec<-JT(DataU[ ,Response],as.ordered(DataU[ ,Treatment]),alternative='decreasing') #50000
...
if (TestDirection=='Both'){
	...
	if (dim(JTTable)[1]==1){
		JTTable<-matrix(c(JTTable[ ,c(1,2,4,6,8)], JTTable[ ,c(3,7)],nrow=1))
		colnames(JTTable)<-c('JT Statistic','Decreasing Trend P-Value','D.Sig',
			'Increasing Trend P-Value','I.Sig','Max Level of Treatment')
	}
